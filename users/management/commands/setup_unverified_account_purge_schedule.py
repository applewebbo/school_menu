"""
Management command to register the unverified-account purge schedule in Django-Q2 (#272).

Modelled on `setup_ai_import_schedule`: the schedule lives in the database, but the
command that creates it is versioned, so a fresh deploy does not depend on somebody
remembering to add it by hand in the admin. Running it again is safe — it updates the
existing row.
"""
...
SCHEDULE_NAME = "Unverified Account Purge"
TASK_PATH = "users.tasks.purge_unverified_accounts"


class Command(BaseCommand):
    """Create, update or remove the Django-Q2 schedule for the account retention."""

    help = "Create or update the Django-Q2 schedule that purges unverified accounts"
    ...
    def _create(self, Schedule, dry_run):
        cron = settings.UNVERIFIED_ACCOUNT_PURGE_SCHEDULE
